...
export function LiveBoardCard({ desk, nextForDesk, nextTwoForDesk }) {
  const d = desk;
  const nextTwo = !d.current ? nextTwoForDesk(d) : [];
  ...
  const badgeText = !d.current ? (idleHasNext ? "Next up" : "Idle") : isStarted ? "Serving" : "Called";
  const badgeColor = !d.current ? C.textFaint : isStarted ? C.teal : C.amber;
  const badgeBg = !d.current ? "transparent" : isStarted ? C.tealSoft : C.amberSoft;
  const borderColor = !d.current ? C.ink700 : isStarted ? C.teal : C.amber;
